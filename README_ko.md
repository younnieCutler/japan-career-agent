...
진로 방향, 이력서·職務経歴書, JD와 기업 정보, 지원 비교, 면접, 다음 행동을 정리할 때 사용하세요. 호스팅 SaaS가 아니라 plugin과 로컬 runtime으로 구성되며, 선택형 로컬 GUI에서 읽기 화면과 재개 가능한 棚卸し 초안을 사용할 수 있습니다. canonical 근거 반영에는 여전히 사용자의 승인이 필요합니다.
...
않습니다. 지금은 소스 메타데이터가 `2.5.1`인데 stable marketplace ref는 아직 `v2.1.1`입니다.

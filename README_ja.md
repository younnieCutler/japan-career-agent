...
キャリアの方向整理、履歴書・職務経歴書、JD と企業情報の確認、求人やオファーの比較、面接練習、次の行動の管理に使えます。ホスティング型SaaSではなく、pluginとローカルruntimeで構成され、読み取り専用のローカルGUIも選べます。
...
参照し、`main` は追跡しません。現在はソースメタデータが `2.4.0` である一方、stable marketplace

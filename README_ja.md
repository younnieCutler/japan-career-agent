...
<p align="center">
  <strong>日本の転職・就職のための evidence-based キャリア意思決定ツール。<br/>
  経歴の記録は自分のマシンに残り、承認なしに何かが事実になることはありません。</strong>
</p>
...
最初から `proposal_id`、`CAREER_VAULT`、`data/pipeline.yml` を理解する必要はありません。必要に
なったときだけ、下のローカル向け workflow で使う概念です。
...
現在はソースメタデータが `2.11.4` である一方、stable marketplace ref はまだ `v2.1.1` です。この

"""Argument parser ownership for the Career Agent CLI.

This module owns only the command/argument contract. Output projection and process exit behavior
stay in ``command_line.py`` so parser growth cannot pull execution concerns into the same module.
"""
...
from vault import today, utc_now
...
def build_parser() -> argparse.ArgumentParser:
    ...
    project_timeline_parser = subparsers.add_parser(
        "project-timeline", help="one project's confirmed work events, as ledger references",
    )
    add_vault_argument(project_timeline_parser)
    project_timeline_parser.add_argument("project_id")
    add_output_format(project_timeline_parser)
    ...
    personal_timeline_parser = subparsers.add_parser(
        "personal-timeline",
        help="full labelled history for one fact key, including superseded records",
    )
    add_vault_argument(personal_timeline_parser)
    personal_timeline_parser.add_argument("--category", required=True, choices=sorted(FACT_CATEGORIES))
    personal_timeline_parser.add_argument("--key", required=True, help="the logical fact key, e.g. jlpt")
    add_output_format(personal_timeline_parser)

...
def _safe_chmod(path: Path, mode: int) -> None:
    try:
        ...
        # Windows and some network filesystems do not implement POSIX mode semantics. The archive
        # remains local-only and gitignored; failure to chmod must not make verification unusable.
        pass
    ...
    if receipt.returncode == 0:
        if receipt.handle:
            return (
                ...
                f"recall: python scripts/agent_observation_pack.py show {receipt.handle}"
            )
